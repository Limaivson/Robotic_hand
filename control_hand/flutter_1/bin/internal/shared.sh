...
function upgrade_flutter () (
  ...
  if [[ ! -f "$SNAPSHOT_PATH" || ! -s "$STAMP_PATH" || "$(cat "$STAMP_PATH")" != "$compilekey" || "$FLUTTER_TOOLS_DIR/pubspec.yaml" -nt "$FLUTTER_TOOLS_DIR/pubspec.lock" ]]; then
    ...
    # conditional allows the majority of flutter_1/dart installations to bypass
    ...
    >&2 echo Building flutter_1 tool...
    ...
    # memory mapped (e.g. on flutter_1 upgrade). For downloading a new dart sdk the folder is moved,
...
# This function is intended to be executed by entrypoints (e.g. `//bin/flutter_1`
...
function shared::execute() {
  ...
  # Test if the flutter_1 directory is a git clone (otherwise git rev-parse HEAD
  # would fail)
