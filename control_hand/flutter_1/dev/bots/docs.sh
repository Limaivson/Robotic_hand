...
function create_offline_zip() {
  ...
  rm -rf flutter_1.docs.zip doc/offline
  (cd ./doc; zip -r -9 -q ../flutter_1.docs.zip .)
}
...
function create_docset() {
  ...
  dashing_pid=$!
  wait $dashing_pid && \
  cp ./doc/flutter_1/static-assets/favicon.png ./flutter_1.docset/icon.png && \
  "$DART" --disable-dart-dev --enable-asserts ./dashing_postprocess.dart && \
  tar cf flutter_1.docset.tar.gz --use-compress-program="gzip --best" flutter_1.docset
  if [[ $? -ne 0 ]]; then
...
function move_offline_into_place() {
  ...
  mkdir -p doc/offline
  mv flutter_1.docs.zip doc/offline/flutter_1.docs.zip
  du -sh doc/offline/flutter_1.docs.zip
  if [[ "$LUCI_BRANCH" == "stable" ]]; then
    echo -e "<entry>\n  <version>${FLUTTER_VERSION_STRING}</version>\n  <url>https://api.flutter.dev/offline/flutter.docset.tar.gz</url>\n</entry>" > doc/offline/flutter_1.xml
  else
    echo -e "<entry>\n  <version>${FLUTTER_VERSION_STRING}</version>\n  <url>https://master-api.flutter.dev/offline/flutter.docset.tar.gz</url>\n</entry>" > doc/offline/flutter_1.xml
  fi
  mv flutter_1.docset.tar.gz doc/offline/flutter_1.docset.tar.gz
  du -sh doc/offline/flutter_1.docset.tar.gz
}
