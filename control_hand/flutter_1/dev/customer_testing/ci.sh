...
# This script does not assume that "flutter_1 update-packages" has been
# run, to allow CIs to save time by skipping that steps since it's
# largely not needed to run the flutter_1/tests tests.
#
# However, we do need to update this directory and the tools directory.
dart pub get
(cd ../tools; dart pub get) # used for find_commit.dart below

# Next we need to update the flutter_1/tests checkout.
#
# We use find_commit.dart so that we pull the version of flutter_1/tests
...
# revved on flutter_1/tests -- if you rerun a passing customer_tests
